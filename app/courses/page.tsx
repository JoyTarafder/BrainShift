  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        {/* Header Banner */}
        <div className="bg-gradient-to-r from-[#0b2545] to-[#1e3a8a] rounded-2xl p-8 lg:p-12 text-white shadow-xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/25 border border-amber-400/40 text-amber-300 text-xs sm:text-sm font-bold shadow-sm">
            <BookOpen className="w-4 h-4 text-amber-400" />
            <span>BrainShift Course Catalog</span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-3 relative">
              <Filter className="w-4 h-4 absolute left-3.5 top-3.5 text-slate-400 pointer-events-none" />
              <select
                value={selectedSubject}
                onChange={(e) => setSelectedSubject(e.target.value)}
                className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0b2545]/20 focus:border-[#0b2545]"
              >
                {subjects.map((sub) => (
        ...
        {/* Course Grid & Section Heading */}
        <section aria-label="Course Catalog" className="space-y-6">
